...
//
// @id = ch.banana.script.estrattocontopatriziati
// @api = 1.0
// @pubdate = 2015-10-16
...
function exec(string) {
	...
	if(verificaConto(conto)){
		...
		var indirizzo1Destinatario = Banana.document.table('Accounts').findRowByValue('Account',conto).value('Address1');
		var capDestinatario = Banana.document.table('Accounts').findRowByValue('Account',conto).value('Zip');
		var luogoDestinatario = Banana.document.table('Accounts').findRowByValue('Account',conto).value('Town');
		var societaDestinatario = Banana.document.table('Accounts').findRowByValue('Account',conto).value('Company');
		var indirizzo2Destinatario = Banana.document.table('Accounts').findRowByValue('Account',conto).value('Address2');
...
function verificaConto(conto){
	...
	if(!Banana.document.table('Accounts').findRowByValue('Account',conto) || !conto){
		Banana.application.addMessage("ATTENZIONE! Conto selezionato non valido.");
		controllo = 0;
		return false;
	}
	else{
